...
export function useVoiceSession(input: UseVoiceSessionInput) {
  ...
  const configureCaptureMode = useCallback(
    ...
    [liveRepliesEnabled, realtimeVoice, resolvedSilenceDurationMs],
  );
  ...
  const ensureMicrophoneAccess = useCallback(async () => {
    ...
    if (!window.isSecureContext) {
      ...
      const message =
        "Voice input requires a secure browser context. Open T3 Code over HTTPS or localhost before using the microphone.";
      dispatch({
        type: "error",
        message,
      });
      throw new Error(`${message} Current host: ${host}.`);
    }

    if (!navigator.mediaDevices?.getUserMedia) {
      dispatch({
        type: "error",
        message: "This browser does not expose microphone access on the current page.",
      });
