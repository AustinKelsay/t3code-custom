  ...
  const beginLocalDispatch = useCallback(
    (options?: {
      ...
      setLocalDispatch((current) => {
        ...
        if (active) {
          const nextDelivery = options?.delivery ?? active.delivery;
          const nextMessageId = options?.messageId ?? active.messageId;
          return active.preparingWorktree === preparingWorktree &&
            active.delivery === nextDelivery &&
            active.messageId === nextMessageId
            ? active
            : {
                ...active,
                preparingWorktree,
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const dispatchThreadCommand = useAtomCommand(threadEnvironment.dispatchCommand, {
    reportFailure: false,
  });
  ...
  const versionMismatchServerLabel =
    hasMultipleRegisteredEnvironments && activeThread
      ...
      : "server";
  const handleRetryQueuedTurn = useCallback(
    async (queueItemId: TurnQueueItemId) => {
      if (!activeThread || isConnecting || activeEnvironmentUnavailable) {
        return;
      }
      setThreadError(activeThread.id, null);
      const result = await dispatchThreadCommand({
        environmentId,
        input: {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThread.id,
          error instanceof Error ? error.message : "Failed to retry queued turn.",
        );
      }
    },
    [
  ...
  const handleRemoveQueuedTurn = useCallback(
    async (queueItemId: TurnQueueItemId) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThread.id,
          error instanceof Error ? error.message : "Failed to remove queued turn.",
        );
      }
    },
    [
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      const bootstrap =
        isLocalDraftThread || baseBranchForWorktree
          ...
          : undefined;
      const turnDispatch = buildComposerTurnDispatch({
        commandId: newCommandId(),
        threadId: threadIdForSend,
        activeTurnId: activeTurnIdForSend,
        messageId: messageIdForSend,
      ...
      beginLocalDispatch({
        ...
        delivery: turnDispatch.delivery,
        messageId: messageIdForSend,
      });
      const startResult = await dispatchThreadCommand({
        environmentId,
        input: turnDispatch.command,
      });
