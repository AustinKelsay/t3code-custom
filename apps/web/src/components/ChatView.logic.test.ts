...
describe("hasServerAcknowledgedLocalDispatch", () => {
  ...
  it("waits for queued dispatch evidence before acknowledging", () => {
  ...
  it("acknowledges accepted and fallback steer dispatch evidence", () => {
    const activeTurnId = TurnId.make("turn-2");
    const messageId = MessageId.make("message-steer");
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({
        session: { ...readySession, status: "running", activeTurnId },
        latestTurn: completedTurn,
      }),
      { delivery: "steer", messageId },
    );
    const common = {
      localDispatch,
      phase: "running" as const,
      latestTurn: completedTurn,
      session: { ...readySession, status: "running" as const, activeTurnId },
      messages: [],
      queuedTurns: [],
      activities: [],
      hasPendingApproval: false,
