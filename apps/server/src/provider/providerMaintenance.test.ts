...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it("uses the Homebrew core OpenCode formula for Homebrew OpenCode installs", () => {
    expect(
      OpenCodeMaintenanceCapabilities.resolve({
  ...
  it.effect("allows npm postinstall scripts when updating npm-managed OpenCode installs", () =>
    Effect.gen(function* () {
      ...
      const packageBinDir = NodePath.join(tempDir, "lib", "node_modules", "opencode-ai", "bin");
      NodeFS.mkdirSync(binDir, { recursive: true });
      NodeFS.mkdirSync(packageBinDir, { recursive: true });
      const packageBinPath = NodePath.join(packageBinDir, "opencode.exe");
      const symlinkPath = NodePath.join(binDir, "opencode");
